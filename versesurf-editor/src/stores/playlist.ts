...
import { fetchMyPlaylistById, savePlaylist, saveSongs } from '@/lib/api'
...
export const usePlaylistStore = defineStore('playlist', () => {
  ...
  async function loadFromBackend(id: string): Promise<boolean> {
    try {
      const data = await fetchMyPlaylistById(id)
      if (!data) {
      ...
      showToast(error instanceof Error ? error.message : 'Failed to load playlist', 'error')
      return false
    }
  }
  ...
  async function saveToBackend(): Promise<boolean> {
    ...
    try {
      ...
      await saveSongs(playlist.value.Id, playlist.value.Songs)
