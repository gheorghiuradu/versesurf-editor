...
export async function fetchSpotifyTrack(url: string): Promise<SpotifyTrackData> {
    ...
    const embedUrl = `https://corsproxy.io/?https://open.spotify.com/embed/track/${trackId}`
...
export async function fetchSpotifyPlaylist(url: string): Promise<SpotifyPlaylistData> {
    ...
    const embedUrl = `https://corsproxy.io/?https://open.spotify.com/embed/playlist/${playlistId}`
    const response = await fetch(embedUrl)
    ...
    const tracks: SpotifyTrackData[] = trackList.map((t: { uri?: string; title?: string; subtitle?: string; audioPreview?: { url?: string } }) => {
        const tUri = t.uri || ''
